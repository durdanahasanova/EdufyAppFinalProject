...
struct OnboardingView: View {
    ...
    private let steps: [OnboardingStep] = [
        OnboardingStep(
            tittle: "Yeni biliklər\nkəşf et",
            describtion: "Yüzlərlə kurs arasından sənə\nən uyğun olanı tap",
            image: "splashfirstimage",
            color: .primaryPurple
        ),

        OnboardingStep(
            tittle: "Öz biliklərini\npaylaş!",
            describtion: "Öz kursunu yarat və biliklərini\nhər kəsə çatdır",
            image: "splashsecondimage",
            color: .greenmedium
        )
                    ...
                    HStack(spacing: 8) {
                        ForEach(0..<steps.count, id: \.self) { index in
                            Capsule()
                                .fill(
                                    currentIndex == index
                                        ? .black : .white.opacity(0.5)
                                )
                                .frame(
                                    width: currentIndex == index ? 36 : 10,
                                    height: 10
                                )
